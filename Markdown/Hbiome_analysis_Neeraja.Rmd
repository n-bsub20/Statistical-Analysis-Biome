...
```{r PCoA plotting using Bray-Curtis distances}
...
```{r PCoA plotting based on Bray-Curtis distances grouped based on Age (Sampling.site+Age was too messy)}
...
```{r PCoA plotting based on Bray-Curtis distances}
...
```{r Statistical testing between groups}
...
```{r Plots of top species, echo = TRUE, fig.height = 10, fig.width = 12}
...
```{r Scalp type, Hair loss, Itchiness, Flaking + Species, echo=TRUE, fig.height=10, fig.width=15}
...
```{r PCoA plotting by Bray-Curtis distances grouped based on Age (Sampling.site + Age was too messy}
...
```{r Are groups different based on Site+Age?}
#Are groups different based on Sampling.site and Age?: Using Permanova testing
...
```{r top bacterial by Species and age, echo = TRUE, fig.height=10, fig.width=12}
...
```{r Compare across only pre-menopause groups, echo=TRUE, fig.height=10, fig.width=12}
#Compare across pre-menopause groups
